...
from settings import window_size, image_background, image_logo, author_email, author_website
from smartgf import sgf_write, sgf_read, sgf_readdir
...
def CreateTables():
    """ 创建数据表 """
    mine.model.createTable()
    expert.model.createTable()
    sihuo.model.createTable()
    dingsi.model.createTable()


class MyFrame(wx.Frame):

    def __init__(self, parent, id=-1, title="围棋打谱 UJS GO", pos=wx.DefaultPosition,
                 size=window_size, style=wx.DEFAULT_FRAME_STYLE | wx.SYSTEM_MENU, ):
        ...
        self.board = GoBoard(wx.Image(image_background), dataBoard, parent=self,
                             size=(600, 600))  # style=wx.FULL_REPAINT_ON_RESIZE)
    ...
    def OnClk(self, event):
        """定时的时间设置事件处理"""
        clktime = wx.GetNumberFromUser(message="ms为单位：", prompt="", caption="定时器的间隔时间", value=self.clktime, min=0,
                                       max=60000)
        if clktime >= 0:
    ...
    def createIcon(self):
        """设置图标"""
        icon = wx.Icon(name=image_logo, type=wx.BITMAP_TYPE_JPEG)
        self.SetIcon(icon)
    ...
    def OnInsgf(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            sgf_read(self.filename, self.mode)
            self.history.Set(self.mode.titles)
        dlg.Destroy()
    ...
    def OnInsgfs(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            sgf_readdir(dlg.GetPath(), self.mode)
            self.history.Set(self.mode.titles)
        dlg.Destroy()
    ...
    def OnAuthor(self, event):
        ...
        wx.MessageBox(author_email, caption="作者邮箱", style=wx.OK)
